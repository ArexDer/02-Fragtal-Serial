...
{
    float dx = (float)(x_max - x_min) / WIDTH;
    float dy = (float)(y_max - y_min) / HEIGHT;
    ...
    // definimos el C
    __m256 c_real = _mm256_set1_ps((float)c.real());
    __m256 c_imag = _mm256_set1_ps((float)c.imag());
    ...
    for (int i = 0; i < WIDTH; i++)
    {
        for (int j = 0; j < HEIGHT; j += 8)
        {
            ...
            */
            ...
            __m256 mk = _mm256_set1_ps((float)iter); // todos van a empezar con 1 --< 7 vees
            ...
            {
                if (j + it >= HEIGHT) continue; // Evitar exceder los límites de la imagen
